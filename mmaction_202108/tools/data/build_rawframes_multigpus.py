...
def extract_frame(vid_item):
    ...
    """
    full_path, vid_path, vid_id, method, task, gpu_used = vid_item
    full_path = full_path
    vid_path = vid_path
    if '/' in vid_path:
    ...
    if task == 'rgb':
        ...
        if args.new_short == 0:
            cmd_rgb = osp.join(
                f"CUDA_VISIBLE_DEVICES={gpu_used} "
                f"denseflow '{full_path}' -b=20 -s=0 -o='{out_full_path}'"
                f' -nw={args.new_width} -nh={args.new_height} -v')
            cmd_flow = osp.join(
                f"CUDA_VISIBLE_DEVICES={gpu_used} "
    ...
    print(f'{task} {vid_id} {vid_path} {method} gpu: {gpu_used} done')
    sys.stdout.flush()
    return True
...
if __name__ == '__main__':
    ...
    print([ random.choice([0,1,2,3]) for i in range(len(vid_list))])
    #print(vid_list)
    #print(vid_list)
    ...
    pool.map(
        ...
        zip(fullpath_list, vid_list, range(len(vid_list)),
            ...
            len(vid_list) * [args.task],[ random.choice([0,1,2,3]) for i in range(len(vid_list))]))
